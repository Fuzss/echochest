...
public class ModLootTableProvider extends LootTableProvider {
    ...
    private class ModBlockLoot extends BlockLoot {
        ...
        protected void addTables() {
            this.add(ModRegistry.ECHO_CHEST_BLOCK.get(), BlockLoot::createNameableBlockEntityTable);
        }

        @Override
        protected Iterable<Block> getKnownBlocks() {
            return ForgeRegistries.BLOCKS.getEntries().stream()
                    .filter(block -> block.getKey().location().getNamespace().equals(ModLootTableProvider.this.modId))
